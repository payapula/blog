...
As always, lets start with a Javascript example, before looking at the output below try to guess what would be logged
into the console.

```javascript
function App(count) {
  console.log('Counter initialised with ' + count);
...
```
Counter initialised with 1 
2
3
4
Counter initialised with 5 
6
7
```
...
The retuned function `print` _closes over_ the variable `count` which from its outer scope. This closing is referred
to as **closure**. 

Please don't get confused with the names of the function, the naming need not necessarily be same, as for an example
...
Here App is returning a function `increment` and we are assiging it to the variable `someRandomName`
...
Alright, MDN is not much helpful here, lets see what w3schools says 
...
the count and logs it. So each time when we call `print`, the count is incremented 
and printed.

_If we are writing logic which uses closures and if we are not careful enough then 
we may fall into a pitfall called...._ 

## Stale Closures

To learn what is stale closures, let us take our same example and modify it further. 

Take a look at this code and guess what would be logged into the console.
...
Try to guess the output, Let me give you some space think.................................... 
...
oh, we are not incrementing the count right ? or do we ?

Lets find out by placing console log inside our `increment` function 
...
Yes we are incrementing and modifying the `count` that is present in the lexical scope
of `increment`. However, the problem lies with the `message` and `log`.

Our `log` function _captured_ the `message` variable and kept it. So, when we increment the count 
the `message` is not updated and our `log` returns the message "Count is 0".
...
And running, would produce the result,
...
What does this _stale closure_ thing has to do in React world?

## Hooks are nothing but Closures!

Lets reproduce the same example we saw in JS world and bring it into the react world, 
...
"Count is 3", but sadly we don't event get anything logged from our example.
...
Note - This is just a contrieved example, you may choose to ignore either of 
...
during the testing, later it would eventually cause a bug and at that time when
in order to fix it we may need to re-write entire logic from scratch !!

This is a big 🚫 NO 🚫 and **MUST BE AVOIDED** at all cost, but how ?

## ESLint Plugin React Hooks

In order to make our life simpler, the react team wrote a ESLint Plugin called 
...
we will get a nice warningm suggesting that we are probably doing something wrong.
...
Keep in mind that stale closure problem does not only affect `useEffect`, we would
...
on custom hooks. So, make a good use of it!
