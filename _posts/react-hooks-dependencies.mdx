...
As always, lets start with a Javascript example, before looking at the output try to guess what would be logged.
...
Please don't get confused with the name of the functions, the names need not 
necessarily be same, as for an example
...
Alright, MDN is not much helpful here, lets see what W3Dchools says 
...
outer scope and instead getting the _staled_ value.
...
we may need to **re-write entire logic** from scratch !!
...
check for all the rules of hooks, So, make a good use of it! 
