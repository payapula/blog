...
A lot of things needs to be taken into consideration while desiging any software application 
in general. In order to build a website, with the component based development approach, 
...
If we are building a dropdown/select component, it must be navigatable via keyboad, should have
...
## A Re-Introduciton to CSS in React

Before we take a dive into Chakra UI, let's revisit on the ways that we can style our React components 
with css.
...
The challenge with this apporach is that the _css_ we are writing is *not scoped* to the component, rather it would
be applied globally. When we are selecting the elements with _classname_ or _ID_s and writing specific styles,
we should be careful to *not repeat the same selectors*. If we are not careful enough then wrong 
styles would be applied to wrong element on a different page. This is because our entire css 
...
_Note: Please ignore the sematics here, as this is just to demonstrate comparision of styles_
...
The awesome part with using Chakra components, the intellisence for the styles are 
so awesome that you need not worry about the casing of styles, also you will get an 
idea of what props are allowed for a component.

The point here is chakra is well documented, and you can easily peek into any
of the components from the repo to learn more about them, and to customize according
to your requirements.

In the above form example with chakra, we have seen a component `<chakra.form/>` which
simply renders HTML `<form>` but the beauty is it can be styled with Chakra's style props.
We can convvert any custom component to receive chakra props with the `chakra` function. 
...
All the chakra UI components are dark mode compatible, the themes are overridable
easily. With the excellent community support and documentation you can build out
...
- [Chakra UI Modal](https://chakra-ui.com/docs/overlay/moda)
